...
### 6.2.1 冒泡排序([Code](./BubbleSort.cpp))
...
### 6.2.2 快速排序([Code](./BubbleSort.cpp))
...
### 6.3.1 直接插入排序([Code](./InsertSort.cpp))
...
### 6.3.2 折半插入排序([Code](./InsertSort.cpp))
此方案和直接插入排序方案基本一致，只是在查找插入位时，用了折半查找，减少了关键字的比较次数，但并为减少元素的移动次数。

### 6.3.3 2-路插入排序([Code](./InsertSort.cpp))

### 6.3.4 希尔排序([Code](./ShellSort.cpp))
